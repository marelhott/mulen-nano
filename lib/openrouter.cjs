...
function imageFromResponse(data) {
  ...
  for (const item of candidates) {
    ...
    if (item?.b64_json) return `data:${item.media_type || item.mime_type || 'image/png'};base64,${item.b64_json}`;
    return String(url);
  }
  return null;
}
...
async function generateImage({ prompt, images = [], model = DEFAULT_IMAGE_MODEL, aspectRatio, resolution, timeoutMs, apiKey }) {
  ...
  if (images.length > 0) body.input_references = images;
  if (aspectRatio && aspectRatio !== 'Original') body.aspect_ratio = aspectRatio;
  if (resolution) body.resolution = resolution;
  const data = await request('/images', body, { timeoutMs: timeoutMs || 300000, apiKey });
